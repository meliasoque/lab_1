...
Our analysis leverages data from the 2018 American National Election Studies (ANES).  This is an observational dataset, based on a sample of respondents drawn from the YouGov platform. The YouGov panel is not nationally representative, and consists of participants who sign up to complete questionnaires in exchange for rewards.  This dataset includes `r nrow_original` individuals. We elect to remove individuals who do not have election turnout values reported in either 2016 or 2018, as well as individuals who either report that they "Do not know", or "Did not respond" to the key _anger_ or _fear_ survey questions. 
...
To compare the two emotions, we identify the respondents who are more angry than afraid, and the respondents who are more afraid than angry.  While it would be tempting to try to retain more information by subtracting one measure from the other, this would inappropriately impose a metric structure on these ordinal variables.  Because we cannot know the cut points between levels, subtracting cannot be justified. 

We find that there are `r sum(anes$emotions == 'More Angry', na.rm = TRUE)` people in the more-anger group and `r sum(anes$emotions == 'More Afraid', na.rm = TRUE)` people in the more-fear group. The right panel of Figure \@ref(fig:plots) shows the fraction of people in each group that went from not voting to voting.

We narrow our research question to the following test: 
...
First, data must be generated via an i.i.d. process.  The ANES 2018 pilot uses a panel of individuals from the YouGov platform. There is a possibility that this introduces dependencies.  For example, participants may tell friends or family members about YouGov, resulting in a cluster of individuals that give similar responses.  Nevertheless, YouGov claims to have millions of users, which suggests that links between individuals should be rare. Second, the data must be measured on a metric scale. Even though our raw data includes ordinal variables, we have transformed them into a metric, binary representation.  A binary variable qualifies as metric as there is only a single interval, which goes from zero to one. This makes the results _technically_ valid, but it raises a reasonable concern about our choice of where to split the data.  Third, and finally, the data must be sufficiently normal so that the CLT produces convergence in distribution to a Gaussian distribution. The small number of new voters can be understood as a positive skew in our turnout variable (skewness = `r round(skewness(anes$new_voter),2)`).  Nevertheless, the large sample size suggests that the sampling distribution of the statistic should be approximately normal via the Central Limit Theorem. 
...
new_voters <- anes %>% 
  filter(emotions %in% c('More Angry', 'More Afraid')) %>% 
  group_by(emotions) %>% 
  summarise(percent =  round(mean(new_voter) * 100, 2))
```

The test yields evidence that angry people are more likely than fearful people to increase voting behavior (t=`r round(test$statistic, 2)`, p=`r round(test$p.value, 2)`).  From a practical perspective, this result appears potentially important.  In the more-anger group, `r new_voters[new_voters$emotions=='More Angry', 'percent']` percent of participants are new voters.  This compares to `r new_voters[new_voters$emotions=='More Afraid', 'percent']` percent in the more-fear group, a difference of `r new_voters[new_voters$emotions=='More Afraid', 'percent'] - new_voters[new_voters$emotions=='More Angry', 'percent']` percentage points.  A difference of this size might typically be considered a small effect, but in a highly competitive, polarized electorate, even a small increase in message effectiveness may swing an election.

Several limitations of our test affect the conclusions that may be drawn from it.  As mentioned above, we are only able to measure associations between emotions and voter turnout, not whether one feature causes another. Additionally, the ANES data is not nationally representative, suggesting that our results may not generalize to the US population.
