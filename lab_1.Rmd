...
Imagine that your organization surveys a set of customers to see how much they like your regular website, and how much they like your mobile website, both measured on 5-point Likert scales.  If you were to run a paired t-test, what consequences would the violation of the metric scale assumption have for your interpretation of the test results?
...
### World Happiness 
The file `datasets/Happiness_WHR` is subsetted from the World Happiness Report, a yearly publication that uses data from the Gallup World Poll surveys.  The variable life ladder is a measure of happiness, described in the FAQ as follows: 
...
### Legislator age
The file `datasets/legislators-current.csv` is taken from the congress-legislators project on Github (https://github.com/unitedstates/congress-legislators).  You would like to test whether Democratic or Republican senators are older.

List all assumptions for a Wilcoxon rank-sum test and evaluate them.

### Its for your health! 
The dataset `wine` can be accessed by installing the `wooldridge` package.  It contains observations of variables related to wine consumption for 21 countries.  You would like to use it to test whether countries have more deaths from heart disease or from liver disease.

List all assumptions for a signed-rank test and evaluate them.

### Positive vibes
The file `./datasets/GSS_religion` is a subset of data from the 2004 General Social Survey (GSS).  The variables `prottemp` and `cathtemp` are measurements of how a respondent feels towards protestants and towards catholics, respectively.  The GSS questions are phrased as follows:
