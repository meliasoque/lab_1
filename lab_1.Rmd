...
You think you know a few things about coins. Namely, that you don't believe in magic, and that you believe that even *within* a pair of tosses, $X_{i}, Y_{i}$, that the outcomes of one coin are independent of the outcomes of the other. What is more, every coin you've ever seen has been a *fair* coin with an equal probability of both outcomes. 

You can formalize, and clarify these beliefs by writing: 
...
After you've written down your beliefs, you aunt who happens to be not only a magician, but also a statistician, claims that $p > 1/2$. 

You design the following test to evaluate your aunt's claim:  You flip the coins three times, and write down that your test statistic is the sum $X_1 + Y_1 + X_2 + Y_2 + X_3 + Y_3$. That is, your test statistic is essentially the nubmer of heads that are shown. 
...
\newpage
## Test Assumptions
For the following questions, your task is to evaluate the assumptions for the given test, using your background knowledge and examining the data.  The test may or may not be the most appropriate test for the scenario.  Do not perform the test.

### World Happiness
The file `datasets/Happiness\_WHR` is subsetted from the World Happiness Report, a yearly publication that uses data from the Gallup World Poll surveys.  The variable life ladder is a measure of happiness, described in the FAQ as follows: 
...
List all assumptions for a two-sample t-test. Then, provide your evaluation of whether this data satisfy these assumptions by reasoning about the sampling process, the measurement strategy, and the data distributions. 

If the data satisfy the assumptions, conduct the test and report on both the statistical and practical significance that you observe.

### Legislators
The file [`datasets/legislators-current.csv`](./datasets/legislators-current.csv) is taken from the congress-legislators project on Github (https://github.com/unitedstates/congress-legislators).  You would like to test whether Democratic or Republican senators are older.

List all assumptions for a Wilcoxon rank-sum test. Then, provide your evaluation of whether this data satisfy these assumptions by reasoning about the sampling process, the measurement strategy, and the data distributions. 
...
List all assumptions for a signed-rank test. Then, provide your evaluation of whether this data satisfy these assumptions by reasoning about the sampling process, the measurement strategy, and the data distributions. 
...
List all assumptions for a paired t-test. Then, provide your evaluation of whether this data satisfy these assumptions by reasoning about the sampling process, the measurement strategy, and the data distributions. (If you produce a histogram to evaluate this data distribution consider the bin width (or widths) that best show important characteristics of the distribution.)
...
Like many modern surveys, the ANES includes survey weights, which are used to correct for differences between how frequently demographic groups appear in the sample compared to the US population (often ultimately relying on US census data). The survey weights make is possible to generalize from the sample of data to the population as a whole. These weights are beyond the scope of our class and you are not expected to utilize them. Even though what you learn might not generalize to the US population, if the data that you posses was generated by an i.i.d. sampling process, you still can learn about the individuals within the sample.

## The Research Question

Use the ANES data to address the following question:

> Did people who think of themselves as Democratic voters or Republican voters experience more difficulty voting in the 2020 election?
...
How do you get from a research question to data? 
...
- What is the best **available** method of measuring this concept? Why have you opted to use this measurement instead of other possibilities? 
